...
export function createCorsConfig(options = {}) {
  ...
  return {
    origin: (requestOrigin) => {
      ...
      // Custom validator
      if (originValidator) {
        const isValid = originValidator(requestOrigin);
        return isValid ? requestOrigin : false;
      }
...
/**
 * CORS middleware for Node.js style handlers
 */
export function corsMiddleware(config = {}) {
  ...
  return (req, res, next) => {
    ...
    if (allowedOrigin) {
      ...
      // Log violation
      logCorsViolation(origin, config.origins || []);
    }
...
/**
 * Apply CORS headers to a Response object (Web API style)
 */
export function applyCorsHeaders(response, origin, config = {}) {
  const corsConfig = createCorsConfig(config);
  const allowedOrigin = corsConfig.origin(origin);
...
/**
 * Check if origin is allowed (simple boolean check)
 */
export function checkOrigin(origin, config = {}) {
  const corsConfig = createCorsConfig(config);
