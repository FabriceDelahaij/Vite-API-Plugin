...
The easiest way to configure compression is using presets:
...
// Or use preset name as string
const { middleware: compress } = createCompressionMiddleware('production');

// Development preset (faster, less aggressive)
const { middleware: compress } = createCompressionMiddleware('development');

// Aggressive preset (maximum compression)
const { middleware: compress } = createCompressionMiddleware('aggressive');

// Minimal preset (light compression, CPU-friendly)
const { middleware: compress } = createCompressionMiddleware('minimal');

// API-optimized preset (tuned for JSON APIs)
const { middleware: compress } = createCompressionMiddleware('api');
```

### Available Presets

| Preset | Use Case | Level | Threshold | Streaming |
|--------|----------|-------|-----------|-----------|
| `production` | Balanced for production | 6 | 1KB | 100KB |
| `development` | Fast iteration | 4 | 5KB | 50KB |
| `aggressive` | Maximum compression | 9 | 512B | 25KB |
| `minimal` | CPU-constrained | 1 | 10KB | Disabled |
| `api` | JSON APIs | 6 | 1KB | 100KB |
| `disabled` | No compression | - | - | - |

### Custom Configuration

Override specific settings from a preset:

```js
const { middleware: compress } = createCompressionMiddleware({
  ...COMPRESSION_PRESETS.production,
  threshold: 2048, // Custom threshold
...
```js
// Use route-level control for security
app.post('/api/auth/login', (req, res) => {
  res.locals.disableCompression = true; // BREACH mitigation
  res.json({ token: 'secret' });
});

// Or use secure preset
createCompressionMiddleware(COMPRESSION_PRESETS.secure);
```

### 6. Monitor Compression Ratio
...
setInterval(async () => {
  ...
  console.log(`Security skips: ${stats.compression.skippedForSecurity}`);
}, 60000); // Every minute
```
...
**New in v2.0:** Disable compression for specific routes using `res.locals.disableCompression`

```js
// Disable compression for authentication endpoints
app.post('/api/auth/login', (req, res) => {
  res.locals.disableCompression = true; // Skip compression
  res.json({ token: 'secret-jwt-token' });
});

// Disable for webhook endpoints
app.post('/api/webhooks/stripe', (req, res) => {
  res.locals.disableCompression = true;
  res.json({ received: true });
});

// Disable for sensitive user data
...
**New in v2.1:** Customize compression settings per route using `res.locals.compression`

```js
// Use aggressive compression for large exports
...
// Use fast compression for time-sensitive endpoints
...
app.get('/api/stream', (req, res) => {
  res.locals.compression = {
    streaming: {
      enabled: true,
      threshold: 50 * 1024  // Stream at 50KB
    }
  };
  // Stream response...
});
...
**New in v2.2:** Per-response cache TTL using `res.locals.cacheTTL`

Different endpoints have different staleness tolerances. Use per-response TTL to optimize cache efficiency:
...
app.get('/api/config', (req, res) => {
  ...
  res.json({ apiVersion: '1.0', features: [...] });
});

// User profile - don't cache (personalized)
app.get('/api/me', authenticate, (req, res) => {
  res.locals.cacheTTL = 0; // No caching
  res.json(req.user);
});
```

**TTL Values:**
- `undefined` - Use global `cache.ttl` (default)
- `0` - Don't cache this response
- `> 0` - Cache for N milliseconds

See `COMPRESSION-PER-RESPONSE-TTL.md` for complete guide.

**New in v2.2:** GraphQL-aware compression optimization

GraphQL responses compress exceptionally well (80-90%) and grow vertically. Automatic optimization:

```js
// Automatically optimized for GraphQL
app.post('/graphql', graphqlHandler);
// → minCompressionRatio: 2% (vs 10% default)
// → streaming.threshold: 32KB (vs 100KB default)

// Custom path - use explicit flag
...
**Benefits:**
- Better compression ratios (80-90% vs 70-80%)
- Earlier streaming for large lists (Relay connections)
- Optimized for vertical growth patterns

See `COMPRESSION-GRAPHQL-OPTIMIZATION.md` for complete guide.

**Use Cases:**
- 🔒 Authentication endpoints (tokens, secrets) - disable compression
- 🔗 Webhook handlers - disable compression
- 👤 User-specific sensitive data - disable compression
- 🎯 Token refresh endpoints - disable compression
- 🛡️ Any route with BREACH concerns - disable compression
- ⚡ Time-sensitive endpoints - fast compression (level 1-3)
- 📦 Large exports - aggressive compression (level 9)
- 🌊 Streaming responses - custom thresholds
- ⏱️ Frequently changing data - short TTL (30s-1m)
- 📋 Stable configuration - long TTL (10m-30m)
- 🚫 Personalized data - no caching (TTL = 0)
...
// Apply custom compression to route groups
app.use('/api/export/*', (req, res, next) => {
  res.locals.compression = {
    level: 9,  // Aggressive compression for exports
    algorithms: ['br', 'gzip']
  };
  next();
});
...
### Internal Services & Zstd Compression

**New in v2.1:** Use `X-Compression-Policy: internal` header for service-to-service communication

Zstd compression offers better speed and compression ratio than gzip, but isn't supported by browsers yet. Internal services can use the `X-Compression-Policy` header to enable Zstd:

```js
// Client: Internal service making API call
...
```bash
# curl example
curl -H "Accept-Encoding: zstd, br, gzip" \
     -H "X-Compression-Policy: internal" \
     https://api.internal/data
```

**Server configuration:**
```js
// Enable zstd in algorithms list
...
**Benefits:**
- 20-30% better compression ratio than gzip
- Faster compression/decompression than brotli
- Ideal for microservices communication
- Reduces bandwidth between internal services

**Legacy support:** `X-Internal-Client: true` header is still supported but deprecated.

### Per-Route Compression Control (Legacy)
...
### Disable Compression for Specific Route (Legacy)
...
- [API Enhancements](./COMPRESSION-API-ENHANCEMENTS.md) - Route-level configuration and X-Compression-Policy header
- [Security Enhancements](./COMPRESSION-SECURITY-ENHANCEMENTS.md) - BREACH mitigation and route-level control
...
createCompressionMiddleware('production');
// → Level 6, 1KB threshold, 100KB streaming

// Development (fast)
createCompressionMiddleware('development');
// → Level 4, 5KB threshold, 50KB streaming

// Aggressive (max compression)
createCompressionMiddleware('aggressive');
// → Level 9, 512B threshold, 25KB streaming

// Minimal (CPU-friendly)
createCompressionMiddleware('minimal');
// → Level 1, 10KB threshold, no streaming

// API-optimized (JSON only)
createCompressionMiddleware('api');
// → Level 6, JSON types only, 100KB streaming
