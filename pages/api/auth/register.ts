...
import { createErrorResponse, createSuccessResponse, validateRequired, isValidEmail } from 'vite-api-routes-plugin';
  ...
  try {
    ...
    if (validationErrors.length > 0) {
      return createErrorResponse(
        ...
        400,
        `Required: ${validationErrors.map((e: { field: string }) => e.field).join(', ')}`
      );
    }
...
async function hashPassword(password: string): Promise<string> {
  // Using Argon2id variant with secure defaults
