...
export default function vitePluginApiRoutes(options = {}) {
  ...
  const {
    ...
    websocket = {
      enabled: true,
      routePrefix: '/ws', // WebSocket routes must be under this path (relative to apiPrefix)
      maxConnections: 1000,
      pingInterval: 30000,
      pingTimeout: 5000,
      maxPayloadSize: 1024 * 1024, // 1MB
      // Origin validation (CORS for WebSocket)
      origins: null, // null = allow all, [] = deny all, ['http://localhost:3000'] = allowlist
  ...
  function loadApiHandlers(root) {
    ...
    // Register WebSocket routes (only routes under the configured WebSocket path)
    if (websocketManager) {
      const wsPath = apiPrefix + websocket.routePrefix;
  ...
  return {
    ...
    configResolved(resolvedConfig) {
      ...
      if (websocket.enabled) {
        websocketManager = new WebSocketManager({
          ...
          pingTimeout: websocket.pingTimeout,
          maxPayloadSize: websocket.maxPayloadSize,
          enableLogging: config.mode === 'development',
          // Origin validation
          origins: websocket.origins,
          validateOrigin: websocket.validateOrigin,
          // UTF-8 validation
          validateUtf8: websocket.validateUtf8,
        });
        console.log('✓ WebSocket support enabled');
        
        if (websocket.validateOrigin && websocket.origins) {
          console.log(`✓ Origin validation enabled (${Array.isArray(websocket.origins) ? websocket.origins.length : 'all'} origins)`);
        }
      }
    ...
    configureServer(server) {
      ...
      if (websocket.enabled && websocketManager) {
        server.httpServer?.on('upgrade', async (req, socket, head) => {
          // Only handle WebSocket upgrades for our API routes, not Vite's HMR
          const url = req.url || '';
          const wsPath = apiPrefix + websocket.routePrefix;
          
          // Check if this is a WebSocket route (not Vite HMR)
          if (url.startsWith(wsPath)) {
            await websocketManager.handleUpgrade(req, socket, head, server, apiPrefix);
          }
      ...
      if (config.mode === 'development') {
        hotReloadManager = new HotReloadManager({
          ...
          debounceMs: 100,
          enableLogging: true,
          // Always exclude WebSocket routes from HMR to prevent connection issues
          excludePatterns: [websocket.routePrefix],
        });
        ...
        server.middlewares.use('/__hmr_status', async (req, res) => {
          ...
          res.end(JSON.stringify({
            ...
            websocket: websocketManager ? websocketManager.getStats() : { enabled: false },
            requestId: requestId.enabled ? {
              enabled: true,
              stats: globalRequestIdManager.getStats(),
              activeRequests: globalRequestIdManager.getActiveRequests().length,
            } : { enabled: false },
          }));
        });
      } else {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        if (requestId.enabled) {
          const requestIdHandler = requestIdMiddleware(requestId);
          requestIdHandler(req, res, () => {
            // Track request start
            if (req.requestId) {
              globalRequestIdManager.startRequest(req.requestId, {
                method: req.method,
                url: req.url,
        ...
        const ip = req.headers['x-forwarded-for']?.split(',')[0] || 
          ...
          try {
            ...
            if (requestId.enabled && req.requestId) {
              globalRequestIdManager.endRequest(req.requestId, {
                statusCode: 500,
                error: error.message,
              });
            }
            ...
            // Track successful request end
            if (requestId.enabled && req.requestId && !res.writableEnded) {
              globalRequestIdManager.endRequest(req.requestId, {
                statusCode: res.statusCode,
              });
            }
